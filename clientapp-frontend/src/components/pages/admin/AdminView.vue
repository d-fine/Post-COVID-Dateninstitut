...
import type { EntityInformation } from '@/generated/openapi';

import { inject, ref } from 'vue'
import { assertDefined } from "@/utils/TypeScriptUtils";
import { InputText, Listbox } from 'primevue';
...
const postProviderUser = async (
    userName: string,
    firstName: string,
    surname: string,
) => {
    const response = await postCreateProviderUser(
...
<template>
    <div class="card flex justify-center gap-4">
        <Form v-slot="$form" :resolver="resolver" :initialValues="initialValues" @submit="onFormCreateUser"
